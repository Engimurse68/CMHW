...
def ff(Re, rr, CBEQN=False):
    ...
    if(CBEQN):
        ...
        result= fsolve(cb,0.02)  # use fsolve to find the fraction factor using Colebrook equation
        return result[0]
    else:
        return 64/Re
    pass
...
def plotMoody(plotPoint=False, pt=(0,0)):
    ...
    ReValsCB=np.logspace(np.log10(4000.0),8.0,100) #initially wrote this with "log10(10e8)"  which didn't work so well...
    ...
    for nRelR in range(len(ffCB)):
        plt.loglog(ReValsCB, ffCB[nRelR], 'k', label=nRelR)  # plot the lines for the turbulent region for each pipe roughness
    ...
    plt.ylabel(r"Friction factor $ f = \frac{64}{Re}$", fontsize=16)
