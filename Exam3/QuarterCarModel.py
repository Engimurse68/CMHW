...
#I don't enjoy how skilled I am getting at changing to PySide6.  Oh well
...
class CarModel():
    ...
    def __init__(self):
        ...
        self.m2 = 45.0  # mass of wheel in kg
        ...
        # k limits based on static compression of 3-6 inches for suspension, 0.75-1.5 inches for tire
        # F = m1*g, compression = F/k  =>  k = F/compression
        self.mink1 = (self.m1 * 9.81) / (6.0 * 0.0254)  # 6 inch max compression
        self.maxk1 = (self.m1 * 9.81) / (3.0 * 0.0254)  # 3 inch min compression
        self.mink2 = (self.m2 * 9.81) / (1.5 * 0.0254)  # 1.5 inch max compression
        self.maxk2 = (self.m2 * 9.81) / (0.75 * 0.0254)  # 0.75 inch min compression
        self.accel = None
        self.accelMax = 0.0
        self.accelLim = 2.0  # limit acceleration to 2g
        self.SSE = 0.0
        self.F_k1 = None  # force in suspension spring
        self.F_c1 = None  # force in dashpot
        self.F_k2 = None  # force in tire spring
...
class CarView():
    def __init__(self, args):
        ...
        self.tab_position.setLayout(tab1_layout)
        self.tabWidget.addTab(self.tab_position, "Position vs. time")
        ...
        self.tab_force.setLayout(tab2_layout)
        self.tabWidget.addTab(self.tab_force, "Force vs. time")
    ...
    def buildScene(self):
        ...
        # Road line at bottom
        road = qtw.QGraphicsLineItem(-150, 100, 150, 100)
        road.setPen(self.penWheel)
        self.scene.addItem(road)

        # Tire spring (k2) between road and wheel
        tireSpring = qtw.QGraphicsLineItem(0, 100, 0, 50)
        tireSpring.setPen(self.penWheel)
        self.scene.addItem(tireSpring)

        # Suspension spring (k1) between wheel and car body
        suspSpring = qtw.QGraphicsLineItem(-15, 20, -15, -55)
        suspSpring.setPen(self.penWheel)
        self.scene.addItem(suspSpring)

        # Dashpot (c1) parallel to suspension spring
        dashpot = qtw.QGraphicsLineItem(15, 20, 15, -55)
        dashpot.setPen(self.penWheel)
        self.scene.addItem(dashpot)

        # Labels
        bodyLabel = qtw.QGraphicsTextItem("m1")
        bodyLabel.setPos(55, -85)
        self.scene.addItem(bodyLabel)

        wheelLabel = qtw.QGraphicsTextItem("m2")
        wheelLabel.setPos(55, 35)
        self.scene.addItem(wheelLabel)

        k1Label = qtw.QGraphicsTextItem("k1")
        k1Label.setPos(-50, -20)
        self.scene.addItem(k1Label)

        c1Label = qtw.QGraphicsTextItem("c1")
        c1Label.setPos(20, -20)
        self.scene.addItem(c1Label)

        k2Label = qtw.QGraphicsTextItem("k2")
        k2Label.setPos(10, 70)
        self.scene.addItem(k2Label)
    ...
    def doPlotForces(self, model=None):
        if model.F_k1 is None:
            return
        ax = self.ax2
        ax.clear()
        t = model.t
        ax.plot(t, model.F_k1, 'b-', label='F_k1 (suspension spring)')
        ax.plot(t, model.F_c1, 'r-', label='F_c1 (dashpot)')
        ax.plot(t, model.F_k2, 'g-', label='F_k2 (tire spring)')
        ax.set_xlabel("time (s)", fontsize='large')
        ax.set_ylabel("Force (N)", fontsize='large')
        ax.set_title("Spring and Dashpot Forces vs. Time")
        ax.legend()
        ax.axvline(x=model.tramp, color='k', linestyle='--')
...
class CarController():
    ...
    def ode_system(self, X, t):
        ...
        # equations of motion for quarter car model
        x1ddot = (1/self.model.m1) * (-self.model.k1*(x1-x2) - self.model.c1*(x1dot-x2dot))
        x2ddot = (1/self.model.m2) * (self.model.k1*(x1-x2) + self.model.c1*(x1dot-x2dot) - self.model.k2*(x2-y))
    ...
    def calculate(self, doCalc=True):
        ...
        self.model.mink1 = (self.model.m1 * 9.81) / (6.0 * 0.0254)
        self.model.maxk1 = (self.model.m1 * 9.81) / (3.0 * 0.0254)
        self.model.mink2 = (self.model.m2 * 9.81) / (1.5 * 0.0254)
        self.model.maxk2 = (self.model.m2 * 9.81) / (0.75 * 0.0254)
    ...
    def calcForces(self):
        ...
        y = np.array([self.model.ymag * (t / self.model.tramp) if t < self.model.tramp
                      else self.model.ymag for t in self.model.t])
        self.model.F_k1 = self.model.k1 * (x1 - x2)
        self.model.F_c1 = self.model.c1 * (x1dot - x2dot)
        self.model.F_k2 = self.model.k2 * (x2 - y)
    ...
    def OptimizeSuspension(self):
        ...
        :return:
        """
        # Step 1: set parameters from GUI without recalculating
        ...
        # unpack optimized values back into model
        self.model.k1, self.model.c1, self.model.k2 = answer.x
        self.le_k1.setText("{:0.2f}".format(self.model.k1))
        self.le_c1.setText("{:0.2f}".format(self.model.c1))
        self.le_k2.setText("{:0.2f}".format(self.model.k2))
        self.view.updateView(self.model)
