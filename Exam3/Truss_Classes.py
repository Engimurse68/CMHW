...
class Link():
    def __init__(self,name="", node1="1", node2="2", length=None, angleRad=None,material='steel', width=1.0, thickness=0.5):
        """ Basic definition of a link contains a name and names of node1 and node2.
        material: 'steel' (density=490 lb/ft^3) or 'aluminum' (density=168 lb/ft^3)
    ...
    def getWeight(self):
        """Calculate weight of link in lbf."""
        if self.length is None:
            return 0.0
        # density in lb/in^3
        density = 0.284 if self.material.lower() == 'steel' else 0.098  # steel or aluminum
        area = self.width * self.thickness  # in^2
        length_in = self.length  # assume length already in inches from input file
        return density * area * length_in  # lbf
...
class TrussView():
    ...
    def setDisplayWidgets(self, args):
        ...
        self.gv.viewport().setAttribute(qtc.Qt.WidgetAttribute.WA_AlwaysShowToolTips, True)
    ...
    def drawLinks(self, truss=None):
        ...
        for l in truss.links:
            ...
            st = 'link name = {}\n'.format(l.name)
            st += 'start: ({:0.3f}, {:0.3f})\n'.format(n1.position.x - offset.x, -(n1.position.y - offset.y))
            st += 'end: ({:0.3f}, {:0.3f})\n'.format(n2.position.x - offset.x, -(n2.position.y - offset.y))
            st += 'length: {:0.3f}\n'.format(l.length)
            st += 'angle: {:0.3f}\n'.format(math.degrees(l.angleRad) if l.angleRad else 0)
            st += 'material: {}\n'.format(l.material)
            st += 'width: {:0.2f} in, thickness: {:0.2f} in\n'.format(l.width, l.thickness)
            st += 'weight: {:0.3f} lbf'.format(l.getWeight())
    ...
    def drawNodes(self, truss=None, scene=None):
        ...
        # calculate total truss weight and reaction loads
        total_weight = sum(l.getWeight() for l in truss.links)
        # simple beam: equal reactions at left and right for symmetric loading
        reaction = total_weight / 2.0

        for n in truss.nodes:
            x = n.position.x - offset.x
            y = (n.position.y - offset.y)
            toolTip = "Node: {}\nVertical Load: {:0.3f} lbf".format(n.name, reaction)
            if n.name.lower() == 'left' or n.name.lower() == 'right':
                ...
                # roller — draw as circle instead of pin (no horizontal constraint)
                self.drawACircle(centerX=x, centerY=y, Radius=10, brush=self.brushPivot,
                                 pen=self.penNode, name=n.name, tooltip=toolTip)
...
class TrussController():
    ...
    def getMousePositionString(self, scenePos, transform):
