...
from Air import *
from X3Q3Dual import dualCycleController
...
class MainWindow(qtw.QWidget, Ui_Form):
    ...
    def selectCycle(self):
        idx = self.cmb_OttoDiesel.currentIndex()
        names = ['Otto', 'Diesel', 'Dual']
        name = names[idx] if idx < len(names) else 'Otto'
        self.gb_Input.setTitle('Input for Air Standard {} Cycle:'.format(name))
        self.controller = [self.otto, self.diesel, self.dual][idx] if idx < 3 else self.otto
        self.controller.updateView()
