...
class rankineView():
    ...
    def selectQualityOrTHigh(self, Model=None):
        ...
        if self.rdo_Quality.isChecked():
            ...
            PCF = 1 if SI else UC.psi_to_bar  # convert GUI pressure to bar for steam lookup
            satProps = Model.steam.getsatProps_p(float(self.le_PHigh.text()) * PCF)
            # step 2: convert saturation temperature to proper units
            tSat = satProps.tsat if SI else UC.C_to_F(satProps.tsat)
    ...
    def setNewPHigh(self, Model=None):
        SI = self.rb_SI.isChecked()
        PCF = 1 if SI else UC.psi_to_bar  # convert GUI input to bar for steam lookup
        satProps = Model.steam.getsatProps_p(float(self.le_PHigh.text()) * PCF)
    ...
    def setNewPLow(self, Model=None):
        ...
        satProps = Model.steam.getsatProps_p(float(self.le_PLow.text()) * PCF)
    ...
    def updateUnits(self, Model=None):
        ...
        if not self.rdo_Quality.isChecked():
            tSat = Model.t_high if Model.t_high is not None else Model.steam.getsatProps_p(Model.p_high).tsat
            pass

        #Step 3. Update the units for labels
        pUnit = 'bar' if Model.SI else 'psi'
        hUnit = 'kJ/kg' if Model.SI else 'BTU/lb'
        wUnit = 'kJ/kg' if Model.SI else 'BTU/lb'
        self.lbl_PHigh.setText("P High ({})".format(pUnit))
        self.lbl_PLow.setText("P Low ({})".format(pUnit))
        self.lbl_H1Units.setText(hUnit)
        self.lbl_H2Units.setText(hUnit)
        self.lbl_H3Units.setText(hUnit)
        self.lbl_H4Units.setText(hUnit)
        self.lbl_TurbineWorkUnits.setText(wUnit)
        self.lbl_PumpWorkUnits.setText(wUnit)
        self.lbl_HeatAddedUnits.setText(wUnit)
        x = self.rdo_Quality.isChecked()
        SI = Model.SI
        self.lbl_TurbineInletCondition.setText(
            "Turbine Inlet: {}{} =".format('x' if x else 'THigh', '' if x else ('(C)' if SI else '(F)')))
