...
class ResistorNetwork_2(ResistorNetwork):
    ...
    def GetKirchoffVals(self,i):
        ...
        self.GetResistorByName('ce').Current = i[3]
        self.GetResistorByName('de').Current = i[1]
        self.GetResistorByName('df').Current = i[4]

        # KCL at node c: current in from bc (i[0]), out through cd (i[2]), out through ce (i[3])
        Node_c = i[0] - i[2] - i[3]

        # KCL at node d: current in from cd (i[2]), out through ad (i[0]),
        #                out through de (i[1]), out through df (i[4])
        Node_d = i[2] - i[0] - i[1] - i[4]

        KVL = self.GetLoopVoltageDrops()  # 3 loop equations (L1, L2, L3)
        KVL.append(Node_c)
        KVL.append(Node_d)
        return KVL
    #endregion
